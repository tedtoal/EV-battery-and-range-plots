...
The next code chunk converts the values in the fuelUsageEff list from the units
specified by fuelUseMetric to the units specified by useMetricInPlots. Then, a
function is defined that uses the piecewise-linear curve defined in fuelUsageEff
to generate gasoline vehicle fuel efficiency from speed.
...
energy use and range computations. We just keep adding functions here as needed.

```{r paged.print=FALSE}

# Define a scale factor to go from Wh to battery percent (or, for example, from Wh/distance to battery percent/distance or
# from Wh/hour (same as W) to battery percent/hour).
scale_Wh_to_BatteryPct = 100/(1000*basicData$batteryCapacity_kWh)
...
kW_Speed_to_batteryPctPerDist = function(kW, Speed) round(scale_Wh_to_BatteryPct*1000*kW/Speed, 3)
...
# kW and Speed can be vectors.
kW_Speed_to_Range = function(kW, Speed) round(100*Speed/(1000*kW*scale_Wh_to_BatteryPct))
...
The next code chunk contains definitions related to drag and grade.
...
The next code chunk computes a bunch of values in dAll derived from the measured
values, and sets some y-axis scale factor variables for use when displaying
multiple y-axes to show values in different units.
...
The next code chunk copies the dAll data frame into two more data frames,
one used for plotting measured values and the other for computing polynomial
approximations.
...
The next code chunk defines limits for plotting SOC (state-of-charge), a percent value.
...
The next code chunk defines speed ranges for plotting.
...
# xlim parameter. Also define number of axis ticks when fineSpeeds is plotted on an axis (usually the x-axis).
...
The next code chunk defines distance ranges for plotting when the fineSpeeds vector is
used for the speeds.
...
The next code chunk computes linear and 3rd-order polynomial approximations
to power as a function of speed. Both approximations use power computed from
measured speed and energy per unit distance to fit the curves. Only the 3rd-
order approximation uses the estimated data points (addForApproximation points).
...
The next code chunk computes linear and power-derived approximations to battery
% per unit distance as a function of speed. The linear approximation uses %
battery per distance computed from measured speed and energy per unit distance
to fit the curve. The derived approximation uses speed along with the 3rd-order
power approximation from above to compute the estimated battery % per distance.
...
The next code chunk computes linear and power-derived approximations to battery
% per hour as a function of speed. The linear approximation uses % battery per
hour computed from measured speed and energy per unit distance to fit the curve.
The derived approximation uses the 3rd-order power approximation from above to
compute the estimated battery % per hour.
...
#                         = 1000*kW*scale_Wh_to_BatteryPct
...
The next code chunk computes linear and power-derived approximations to range as
a function of speed. The linear approximation uses range computed from measured
speed and energy per distance (and battery capacity) to fit the curve. The
derived approximation uses speed along with the 3rd-order power approximation
from above (and battery capacity) to compute the estimated range.
...
#       range = 100*speed/(1000*kW*scale_Wh_to_BatteryPct))
...
The next code chunk computes power-derived approximation to distance per unit price
as a function of speed, using speed along with the 3rd-order power approximation
from above (and battery capacity).
...
The next code chunk computes power-derived approximation to cost per unit distance
as a function of speed, using speed along with the 3rd-order power approximation
from above (and battery capacity).
...
The next code chunk computes power-derived approximation to cost for fixed distance
as a function of speed, using speed along with the 3rd-order power approximation
from above (and battery capacity).
...
The next code chunk uses the 3rd-order power approximation above to compute
estimates of total power at speeds given by fineSpeeds.
...
# lines.
maxChargeMinutes_charging = 10*ceiling(max(dfChargeCurve$Minutes)/10)
nTicksChargeMinutes_charging = maxChargeMinutes_charging/5

# Define similar constants when dfChargeCurve$Power is plotted as battery %/hour.
maxChargePower_battPctPerHr_charging = 100*ceiling(max(dfChargeCurve$Power)*1000*scale_Wh_to_BatteryPct/100)
nTicksChargePower_battPctPerHr_charging = maxChargePower_battPctPerHr_charging/50

# Define similar constants when dfChargeCurve$Power is plotted as kW.
maxChargePower_kW_charging = 50*ceiling(max(dfChargeCurve$Power)/50)
nTicksChargePower_kW_charging = maxChargePower_kW_charging/25

# Maximum battery energy in non-degraded battery shown by charge curve.
maxBatteryEnergy_charging = max(dfChargeCurve$TotalEnergy)
nTicksBatteryEnergy_charging = maxBatteryEnergy_charging/5

# Define a scale factor to go from undegraded battery energy to state-of-charge percent, so that one curve can use two different y-axes
# for the two units of energy.
yscaleBatteryEnergy_to_SOCpct_charging = 100/max(dfChargeCurve$TotalEnergy)
...
yscaleDistPerUnitFuel_to_DistPerCost_fineSpeed = maxDistPerCost_fineSpeed/maxDistPerUnitFuel_fineSpeed_1
...
yscaleCostPerDist_to_DistPerUnitFuel_fineSpeed = maxCostPerDist_fineSpeed/maxDistPerUnitFuel_fineSpeed_2
...
yscaleDistPerUnitFuel_to_DistPerCostForDistHours = maxCostForDistHours_fineSpeed/maxDistPerUnitFuel_fineSpeed_3
...
plotCT_PvsS = function(isPDF=FALSE)
    ...
    svpar = par(mai=par("mai")+c(0, 0.1, 0.25, 0.6), mgp=c(2.5, 1, 0))
    ...
    at = 1000*labels * scale_Wh_to_BatteryPct
...
plotR_DC_DEvsS = function(isPDF=FALSE)
    ...
    svpar = par(mai=par("mai")+c(0, 0.1, 0.3, 1.0), mgp=c(2.75, 1, 0))
...
plotDvsSCE = function(isPDF=FALSE)
    ...
    svpar = par(mai=par("mai")+c(0, 0.1, 0.2, 0.6), mgp=c(2.5, 1, 0))
...
plotDPMvsS = function(isPDF=FALSE)
    ...
    svpar = par(mai=par("mai")+c(0, 0.1, 0.5, 0.6), mgp=c(2.5, 1, 0))
    ...
    # Right-side y-axis.
    labels = pretty.good2(ylim_right, yaxp_right[3])
    at = labels * yscaleDistPerUnitFuel_to_DistPerCost_fineSpeed
    axis(side=4, at=at, labels=labels, las=2)
    ...
    # used in plotting.
    DistPerUnitFuel = dt_fineSpeeds$distPerUnitFuel * yscaleDistPerUnitFuel_to_DistPerCost_fineSpeed
...
plotMPDvsS = function(isPDF=FALSE)
    ...
    # Right-side y-axis.
    labels = pretty.good2(ylim_right, yaxp_right[3])
    at = labels * yscaleCostPerDist_to_DistPerUnitFuel_fineSpeed
    axis(side=4, at=at, labels=labels, las=2)
    mtext(tprintf("Fuel consumption (~fuel_eff@)"), side=4, line=2.5)
    ...
    # Plotting DistPerUnitFuel means translating from the desired right-side y-axis to the left-side y-axis
    # used in plotting.
    DistPerUnitFuel = dt_fineSpeeds$distPerUnitFuel * yscaleCostPerDist_to_DistPerUnitFuel_fineSpeed
    lines(dt_fineSpeeds$speed, DistPerUnitFuel, col=col_dist_per_fuel, lty=lty_dist_per_fuel, lwd=lwd_dist_per_fuel)
...
plotMvsS = function(isPDF=FALSE)
    ...
    # Right-side y-axis.
    labels = pretty.good2(ylim_right, yaxp_right[3])
    at = labels * yscaleDistPerUnitFuel_to_DistPerCostForDistHours
    axis(side=4, at=at, labels=labels, las=2)
    mtext(tprintf("Fuel consumption (~fuel_eff@)"), side=4, line=2.5)
    ...
    # Plotting DistPerUnitFuel means translating from the desired right-side y-axis to the left-side y-axis
    # used in plotting.
    DistPerUnitFuel = round(dt_fineSpeeds$distPerUnitFuel * yscaleDistPerUnitFuel_to_DistPerCostForDistHours, 3)
    lines(dt_fineSpeeds$speed, DistPerUnitFuel, col=col_dist_per_fuel, lty=lty_dist_per_fuel, lwd=lwd_dist_per_fuel)
...
plotRvsSG = function(isPDF=FALSE)
    ...
    svpar = par(mai=par("mai")+c(0, 0.1, 0.85, 0.5), mgp=c(2.5, 0.6, 0))
    ...
    mtext(makeStringList("~EVdesc@", "~conditions@", "~totalWeight@", "~battDeg@", "~SOC100@", "~effFactor@",
        ...
        "Lift Power = vehicleWeight_kg x N_per_kg x Speed_kmps x fracGrade x conversionEfficiencyFactor",
...
plotRvsSV = function(isPDF=FALSE)
    ...
    mtext(makeStringList("~EVdesc@", "~conditions@", "~totalWeight@", "~battDeg@", "~SOC100@", "~effFactor@",
        ...
        "Lift Energy = vehicleWeight_kg x N_per_kg x ElevChg_m x kWh_per_Nm x (if ElevChg_m < 0: regenEfficiency)",
        c("3rd-order approx. for Flat Road ", apx$Speed_Power$ord3$textplus), cex=cex),
        side=3, line=0.3, adj=0, cex=cex)
...
plotCT_PvsS_AIR = function(isPDF=FALSE)
    ...
    svpar = par(mai=par("mai")+c(0, 0.1, 0.45, 0.6), mgp=c(2.5, 1, 0))
...
plotCD_EDvsS_WIND = function(isPDF=FALSE)
    ...
    for (windSpeed in as.character(windSpeeds))
        ...
        mtx[windSpeed,] = scale_Wh_to_BatteryPct*1000*totalPower_kW/fineSpeeds
        }
...
plotCT_PvsS_WIND = function(isPDF=FALSE)
    ...
    for (windSpeed in as.character(windSpeeds))
        ...
        airSpeeds_mph = fineSpeeds_mph + windSpeeds_mph[windSpeed]
        totalPower_kW = baselinePower_fineSpeeds + dragPower_kW_TestVehicle_in_mph_metric(airSpeeds_mph, rho)
        battPctPrHr = scale_Wh_to_BatteryPct*1000*totalPower_kW
    ...
    plot(NA, type='n', xlim=xlim, xaxp=xaxp, ylim=ylim_left, yaxp=yaxp_left, las=2, main="",
        xlab=tprintf("Speed on flat road (~speed@)"),
        ylab=tprintf("Battery consumption (~battery_pwr@)"))
    title(tprintf("~battery_pwr_long` at various car and wind speeds"), line=5, cex.main=1.1)
    cex = 0.75
    mtext(makeStringList("~EVdesc@", "~conditions@", "~totalWeight@", "~drag@", "~areaFrontal@", "~airdens@", "~battDeg@", "~SOC100@", "~effFactor@",
        c("3rd-order approx. for Flat Road ", apx$Speed_Power$ord3$textplus), cex=cex),
        side=3, line=0.3, adj=0, cex=cex)
...
plotR_DC_DEvsS_WIND = function(isPDF=FALSE)
    ...
    svpar = par(mai=par("mai")+c(0, 0.1, 0.55, 1.2), mgp=c(2.5, 1, 0))
...
plotCPvsSOC = function(isPDF=FALSE)
    ...
    xlim = c(0, 100)
    xaxp = c(xlim, 20)
    ylim_left = c(0, maxChargePower_battPctPerHr_charging)
    yaxp_left = c(ylim_left, nTicksChargePower_battPctPerHr_charging)
    ylim_right = c(0, maxChargePower_kW_charging)
    yaxp_right = c(ylim_right, nTicksChargePower_kW_charging)

    svpar = par(mai=par("mai")+c(0, 0.1, 0, 0.6), mgp=c(2.5, 1, 0))
    
    plot(NA, type='n', xlim=xlim, xaxp=xaxp, ylim=ylim_left, yaxp=yaxp_left, las=2, main="",
        xlab=tprintf("State-of-Charge (SOC, %)"),
        ylab=tprintf("Charge rate (~battery_pwr@)"))
    title(tprintf("Maximum possible battery charge rate at different states-of-charge"), line=2.25, cex.main=1.1)
    mtext(tprintf(
    ...
    at = 1000*labels * scale_Wh_to_BatteryPct
    axis(side=4, at=at, labels=labels, las=2)
    mtext(tprintf("Charge power (~power@)"), side=4, line=2.5)

    par(xaxp=xaxp)
    par(yaxp=yaxp_left)
    grid(lwd=lwd_grid, lty=lty_grid, col=col_grid)
    
    lines(dfChargeCurve$SOCpct, dfChargeCurve$Power*1000*scale_Wh_to_BatteryPct, lwd=2)
    par(svpar)
    }
...
plotCPvsCT = function(isPDF=FALSE)
    ...
    plot(NA, type='n', xlim=xlim, xaxp=xaxp, ylim=ylim_left, yaxp=yaxp_left, las=2, main="",
        ...
        ylab=tprintf("Charge rate (~battery_pwr@)"))
    title(tprintf("Maximum possible battery charge rate as time passes starting from empty battery"), line=2.25, cex.main=1.1)
    mtext(tprintf(
        "~chargeCurveDescription@",
        "\n",
        "~chargeCurveCitation@"),
    ...
    lines(dfChargeCurve$Minutes, dfChargeCurve$Power*1000*scale_Wh_to_BatteryPct, lwd=2)
    par(svpar)
    }
...
plotE_SOCvsCT = function(isPDF=FALSE)
    ...
    ylim_left = c(0, maxSOCpct_axis)
    yaxp_left = c(ylim_left, nTicksSOCpct_axis)
    ylim_right = c(0, maxBatteryEnergy_charging)
    yaxp_right = c(ylim_right, nTicksBatteryEnergy_charging)

    svpar = par(mai=par("mai")+c(0, 0.1, 0, 0.6), mgp=c(2.5, 1, 0))
    
    plot(NA, type='n', xlim=xlim, xaxp=xaxp, ylim=ylim_left, yaxp=yaxp_left, las=2, main="",
        xlab=tprintf("Charge time (minutes)"),
        ylab=tprintf("State-of-Charge (SOC, %)"))
    title(tprintf("Largest possible state-of-charge as charging time passes starting from empty battery"), line=2.25, cex.main=1.1)
    mtext(tprintf(
        "~chargeCurveDescription@",
        "\n",
        "~chargeCurveCitation@"),
    ...
    # Right-side y-axis.
    labels = pretty.good2(ylim_right, yaxp_right[3])
    at = labels * yscaleBatteryEnergy_to_SOCpct_charging
    axis(side=4, at=at, labels=labels, las=2)
    mtext(tprintf("Battery energy (~energy@)"), side=4, line=2.5)
    ...
    lines(dfChargeCurve$Minutes, dfChargeCurve$TotalEnergy*yscaleBatteryEnergy_to_SOCpct_charging, lwd=2)
...
plotCTvsSOC = function(isPDF=FALSE)
    ...
    xlim = c(0, 100)
    xaxp = c(xlim, 20)
    ylim = c(0, maxChargeMinutes_charging)
    yaxp = c(ylim, nTicksChargeMinutes_charging)
    
    svpar = par(mai=par("mai")+c(0, 0.1, 0, 0), mgp=c(2.5, 1, 0))
    
    plot(NA, type='n', xlim=xlim, xaxp=xaxp, ylim=ylim, yaxp=yaxp, las=2, main="",
        xlab=tprintf("State-of-Charge (SOC, %)"),
        ylab=tprintf("Charge time (minutes)"))
    title(tprintf("Shortest possible time to charge to different states-of-charge"), line=2.25, cex.main=1.1)
    mtext(tprintf(
        "~chargeCurveDescription@",
        "\n",
        "~chargeCurveCitation@"),
