...
The first code chunk here contains values to be set by the user to
control the input directory and output characteristics.
...
The next code chunk contains the results of measurements that must be
made, plus vehicle-related data and the directory and filename to be
used for PDF file output.

Review the following values and make changes as needed.

To change this code to a different vehicle, edit the values in this and
the following code chunk.

```{r paged.print=FALSE}

# Measured energy use per distance at different speeds in both directions on some road. For units, see basicData_metric and
...
# Verify that the SOCpct is a simple rounding based on TotalEnergy and 75 KWh battery.
...
# battery capacity.
if (any(dfChargeCurve$SOCpct != round(100*dfChargeCurve$TotalEnergy/75)))
...
#   ----        --------    ------
#   distance    mile        km
#   dist_plural miles       km
#   speed       mph         km/h
#   power       Kw          Kw
#   energy use  wh/mile     wh/km   Note: this must be in Wh and the same distance units as speed
#   temperature °F          °C
#   elevation   ft          m
#   barometer   inHg        hPA
#   weight      lb          kg
...
unitsTable = list(
    distance=ifelse(useMetricInPlots, "km", "mile"),
    dist_plural=ifelse(useMetricInPlots, "km", "miles"),
    speed=ifelse(useMetricInPlots, "km/h", "mph"),
    power="Kw",
    energyUse=ifelse(useMetricInPlots, "wh/km", "wh/mile"),
    ...
    costPerDist=ifelse(useMetricInPlots, "Eu/km", "$/mi"))# "€" doesn't display in PDF files. Change if another currency needed.
```

The next code chunk contains a list of the air conditions to use when
making certain plots, and one of these conditions (named "testCond") is
the air conditions that were present when the vehicle was tested to
gather the basicData.
...
The next code chunk contains physical constants and one-line functions
for manipulating physical values. It should not be necessary to change
these.
...
The next code chunk contains "includes" of my standard include files.
Normally I place this first in the file, but I wanted to define
constants the user should see first above.
...
The next code chunk contains additional constant definitions affecting
plot colors and line styles. Change these if you wish.
...
The next code chunk adds to basicData some values derived from other
values in basicData.
...
# Define a scale factor to go from battery energy to state-of-charge percent, so that one curve can use two different y-axes
# for the two units of energy.
yscaleBatteryEnergy_to_SOCpct = 100/basicData$batteryCapacity_KWh

```

The next code chunk makes three new versions of the basicData list: one
in imperial units, one in metric units, and one in the units being used
for plotting.
...
The next code chunk contains definitions of functions for various power,
energy use and range computations.
...
The next code chunk contains definitions of more general-purpose
functions.
...
# The substitution is applied to each argument that is of type character. All ... arguments (after such substitution) are
# concatenated using paste0 and the resulting single string is returned.
tprintf = function(...)
    {
    L = list(...)
    resS = ""
...
The next code chunk places the measured speed and wh/mi data into a data
frame, using the units used for plotting.
...
dt$BatteryPctPerUnitDistance = round((dt$WhPerUnitDistance/1000)/(basicData$batteryCapacity_KWh/100), 2)

# Define a scale factor to go from Wh per unit distance to battery percent per unit distance, so that one curve can use
# two different y-axes for the two units of energy.
i = which.max(dt$BatteryPctPerUnitDistance)
yscale_Wh_to_BatteryPct_PerUnitDistance = dt$BatteryPctPerUnitDistance[i]/dt$WhPerUnitDistance[i]

# Compute the distance per 1% battery used.
dt$DistancePerBattery1pct = round((basicData$batteryCapacity_KWh/100)/(dt$WhPerUnitDistance/1000), 2)

# Compute the inverse, distance per unit energy.
dt$DistancePerKWh = round(1000/dt$WhPerUnitDistance, 2)
...
# Define two scale factors, one to go from distance per battery 1% to range, the other to go from distance per KWh to range.
# This allows use of a single curve with up to three different y-axes: range, distance per battery 1%, and distance per KWh,
# all axes placed properly using these scale factors.
i = which.max(dt$range)
yscale_DistancePerBattery1pct_to_Range = dt$range[i]/dt$DistancePerBattery1pct[i]
yscale_DistancePerKWh_to_Range = dt$range[i]/dt$DistancePerKWh[i]
...
dt$electCostPerUnitDist = round(1/dt$electDistPerUnitPrice, 3)
...
# Compute fast distance travelled per unit of currency and its inverse.
dt$gasDistPerUnitPrice = round(dt$distPerUnitFossilFuel/basicData_plot$avgFuelCostPerUnitOfFuel, 2)
dt$gasCostPerUnitDist = round(1/dt$gasDistPerUnitPrice, 3)
...
# Compute electricity and equivalent gasoline cost per energyCostDist distance.
dt$electCostForFixedDistance = round(basicData_plot$avgElecCostPerKWh*energyCostDist*dt$WhPerUnitDistance/1000, 2)
dt$gasCostForFixedDistance = round(basicData_plot$avgFuelCostPerUnitOfFuel*energyCostDist/dt$distPerUnitFossilFuel, 2)
...
Determine values to use to set the plotting limits and number of ticks
for various plotted data.

```{r paged.print=FALSE}
# Define a constant for axis upper limit for plots of dt$speed. Round it up to next-highest multiple of 10, it will work better
# for upper axis limit. Also define a constant for number of tick marks and grid lines when dt$speed is plotted on an axis.
maxMeasuredSpeed_axis = 10*ceiling(max(dt$speed)/10)
nTicksMeasuredSpeed_axis = maxMeasuredSpeed_axis/5

# Define similar constants when dt$BatteryPctPerUnitDistance is plotted on an axis.
maxBatteryPctPerUnitDistance_axis = ceiling(max(dt$BatteryPctPerUnitDistance))
nTicksBatteryPctPerUnitDistance_axis = maxBatteryPctPerUnitDistance_axis*10

# Define similar constants when dt$WhPerUnitDistance is plotted on an axis.
...
# Define similar constants when dt$kw is plotted on an axis.
maxKw_axis = 10*ceiling(max(dt$kw)/10)
nTicksKw_axis = maxKw_axis/5

# Define similar constants when dt$range is plotted on an axis.
maxRange_axis = 100*ceiling(max(dt$range)/100)
nTicksRange_axis = maxRange_axis/50

# Define similar constants when dt$DistancePerBattery1pct is plotted on an axis.
maxDistancePerBattery1pct_axis = ceiling(max(dt$DistancePerBattery1pct))
nTicksDistancePerBattery1pct_axis = 2*maxDistancePerBattery1pct_axis

# Define similar constants when dt$DistancePerKWh is plotted on an axis.
maxDistancePerKWh_axis = ceiling(max(dt$DistancePerKWh))
nTicksDistancePerKWh_axis = 2*maxDistancePerKWh_axis

# Define similar constants when SOC (state-of-charge), a percent value, is plotted on the y-axis.
maxSOCpct_axis = 100
nTicksSOCpct_axis = 20

# Define similar constants when battery energy is plotted on the RIGHT-SIDE y-axis and corresponding state-of-charge in percent is
# plotted on the LEFT-SIDE y-axis. Battery energy maxes out at the specified DEGRADED capacity. We interpret 100% state-of-charge
# as battery at DEGRADED CAPACITY. Note that this means that the grid lines for the state-of-charge on the left y-axis will generally
# NOT line up with the tick marks on the battery energy on the right y-axs. We can't do anything about this.
maxBatteryEnergy_right_axis = 25*ceiling(basicData$batteryCapacity_KWh/25)
nTicksBatteryEnergy_right_axis = maxBatteryEnergy_right_axis/5

# Define similar constants when dt$electDistPerUnitPrice is plotted on the LEFT-SIDE y-axis.
maxDistPerUnitPrice_left_axis = 10*ceiling(max(c(dt$electDistPerUnitPrice, dt$gasDistPerUnitPrice))/10)
nTicksDistPerUnitPrice_left_axis = maxDistPerUnitPrice_left_axis/5

# Define similar constants when dt$distPerUnitFossilFuel is plotted on the RIGHT-SIDE y-axis with dt$electDistPerUnitPrice plotted
# on the LEFT-SIDE y-axis. This is computed so that the grid lines will line up with BOTH AXES, which may lead to poor scaling for
# the right-side axis, just however it turns out. Also define a scale factor to go from distance per unit fossil fuel to distance
# per unit price, so that one y-axis can provide correct scale for two curves, one for electric vehicles and one for fossil fuel
# vehicles.
maxDistPerUnitFossilFuel_right_axis_1 = maxDistPerUnitPrice_left_axis *
    ceiling(max(dt$distPerUnitFossilFuel)/maxDistPerUnitPrice_left_axis)
nTicksDistPerUnitFossilFuel_right_axis_1 = nTicksDistPerUnitPrice_left_axis
yscaleDistancePer_FossilFuel_to_UnitPrice = maxDistPerUnitPrice_left_axis/maxDistPerUnitFossilFuel_right_axis_1

# Define similar constants when dt$electCostPerUnitDist or dt$gasCostPerUnitDist is plotted on the LEFT-SIDE y-axis.
maxCostPerUnitDist_left_axis = 5*ceiling(100*max(c(dt$electCostPerUnitDist, dt$gasCostPerUnitDist))/5)/100
nTicksCostPerUnitDist_left_axis = 100*maxCostPerUnitDist_left_axis

# Define similar constants when dt$distPerUnitFossilFuel is plotted on the RIGHT-SIDE y-axis with the immediately above values
# plotted on the LEFT-SIDE y-axis. This is computed so that the grid lines will line up with BOTH AXES, which may lead to poor
# scaling for the right-side axis, just however it turns out. Names are the same as earlier with _1 changed to _2, same thing
# is being plotted, but these are different plots and the left-side axes are different. Similar scale factor but name is
# different because left-axis units are  different.
maxDistPerUnitFossilFuel_right_axis_2 = maxCostPerUnitDist_left_axis *
    ceiling(max(dt$distPerUnitFossilFuel)/maxCostPerUnitDist_left_axis)
nTicksDistPerUnitFossilFuel_right_axis_2 = nTicksCostPerUnitDist_left_axis
yscaleCostPerUnitDist_to_DistPerUnitFossilFuel = maxCostPerUnitDist_left_axis/maxDistPerUnitFossilFuel_right_axis_2

# Define similar constants when any of dt$electCostForFixedDistance, dt$electCostForHours, dt$gasCostForFixedDistance, or dt$gasCostForHours
# is plotted on the LEFT-SIDE y-axis.
maxCostForDistHours_left_axis = 10*ceiling(max(
    c(dt$electCostForFixedDistance, dt$electCostForHours, dt$gasCostForFixedDistance, dt$gasCostForHours))/10)
nTicksCostForDistHours_left_axis = maxCostForDistHours_left_axis/5

# Define similar constants when dt$distPerUnitFossilFuel is plotted on the RIGHT-SIDE y-axis with the immediately above values
# plotted on the LEFT-SIDE y-axis. This is computed so that the grid lines will line up with BOTH AXES, which may lead to poor
# scaling for the right-side axis, just however it turns out. Names are the same as earlier with _1 changed to _3, same thing
# is being plotted, but these are different plots and the left-side axes are different. Similar scale factor but name is
# different because left-axis units are  different.
maxDistPerUnitFossilFuel_right_axis_3 = maxCostForDistHours_left_axis *
    ceiling(max(dt$distPerUnitFossilFuel)/maxCostForDistHours_left_axis)
nTicksDistPerUnitFossilFuel_right_axis_3 = nTicksCostForDistHours_left_axis
yscaleDistancePer_FossilFuel_to_CostForDistHours = maxCostForDistHours_left_axis/maxDistPerUnitFossilFuel_right_axis_3

```

Create dt_batteryPlots to use with plotting battery range, and
associated variables for determining plot limits and number of ticks.
...
# Define constants for axis lower and upper limits for plots of dt_batteryPlots$speed on an axis. For minimum speed,
# round the actual minimum down to next lower multiple of 5. For maximum, use same maximum as for earlier plots with speed.
# Also define a constant for number of tick marks and grid lines when speed is along an axis in battery plots, spacing it
# every 5 mph or km/h.
...
Additional variables for axis limits of certain plots.
...
# and also convert to mph for use in functions requiring imperial units. Define number of axis ticks. 
...
Axis limit and number of ticks for charge curve variables.

```{r paged.print=FALSE}
dfChargeCurve$Minutes = round(dfChargeCurve$Seconds/60, 2)

# Define a constant for axis upper limit for plots of dfChargeCurve$Minutes on an axis. Round it up to next-highest
# multiple of 10, it will work better for upper axis limit. Also define a constant for number of tick marks and grid
# lines.
maxChargeMinutes_axis = 10*ceiling(max(dfChargeCurve$Minutes)/10)
nTicksChargeMinutes_axis = maxChargeMinutes_axis/5

# Define similar constants when dfChargeCurve$Power is plotted.
maxChargePower_axis = 50*ceiling(max(dfChargeCurve$Power)/50)
nTicksChargePower_axis = maxChargePower_axis/10

```

The next code chunk computes approximations to several curves based on
columns in dt. The coefficients are stored in list apx (approximation).
The list contains sublists, one for each parameter that has one or more
approximations, named for the parameter (e.g. SpeedEnergyUsed for an
approximation of energy consumption based on speed). Within the
parameter list is one or more sublists, one for each approximation. The
names for each sublist are "ord1", "ord2", "ord3" for approximations
that are linear or first order (ord1), quadratic or second order (ord2),
or third order (ord3). Within the ord list are the coefficients of the
approximation (A, B, C, etc., or for linear I (intersect) and M
(slope)). In some cases scaling constants are used to make a more
easier-to-read formula, in which case there may be members such as
'divisor'. List member 'text' is a string representation of the
approximation formula, and member 'expr' is an R expression object for
the formula.
...
# Compute linear approximation for SpeedEnergyUsed, energy consumption as a function of speed.
...
apx$SpeedEnergyUsed$ord1 = coefs

# Compute quadratic approximation for SpeedEnergyUsed, energy consumption as a function of speed.
...
apx$SpeedEnergyUsed$ord2 = coefs
...
The next code chunk uses the approximations above to compute some
estimates of power values at speeds given by fineSpeeds.
...
Plot battery percent per unit distance and watt-hours per unit distance at different speeds.
...
plotCD_EDvsS = function(isPDF=FALSE)
    ...
    ylim_left = c(0, maxBatteryPctPerUnitDistance_axis)
    yaxp_left = c(ylim_left, nTicksBatteryPctPerUnitDistance_axis)
    ylim_right = c(0, maxWhPerUnitDistance_axis)
    yaxp_right = c(ylim_right, nTicksWhPerUnitDistance_axis)
    svpar = par(mai=par("mai")+c(0,0,0.25,0.5), mgp=c(2.5, 1, 0))

    plot(NA, type='n', xlim=xlim, ylim=ylim_left, xaxp=xaxp, yaxp=yaxp_left, las=2, cex.main=ifelse(isPDF, 1, 1.1),
        main=tprintf("Battery Charge Used and Energy Used per ~distance`, ~description@"),
        xlab=tprintf("Speed on Flat Road (~speed@)"),
        ylab=tprintf("Battery % per ~distance@"))
    mtext(tprintf("this shows raw measured data, and includes linear and quadratic approximations\n",
        "(assume battery degradation of ~batteryDegradation_pct@% for capacity of ~batteryCapacity_KWh@ KWh, ",
        "100% state-of-charge is ~batteryCapacity_KWh@ KWh)"),
        side=3, line=ifelse(isPDF, 0.5, 0.3), cex=0.9)

    # Right-side y-axis.
    labels = pretty(ylim_right, yaxp_right[3])
    at = labels * yscale_Wh_to_BatteryPct_PerUnitDistance
    axis(side=4, at=at, labels=labels, las=2)
    mtext(tprintf("watt-hours per ~distance@"), side=4, line=2.5)

    par(xaxp=xaxp)
    par(yaxp=yaxp_left)
    grid(col="darkgray", lwd=0.5, lty="solid")
    points(dt$speed, yscale_Wh_to_BatteryPct_PerUnitDistance*dt$WhPerUnitDistance, pch=20)
    lines(dt$speed, yscale_Wh_to_BatteryPct_PerUnitDistance*dt$WhPerUnitDistance, lwd=2)
    points(dt$speed, yscale_Wh_to_BatteryPct_PerUnitDistance*dt$WhPerUnitDistance_dir2, pch=20, col="gray")
    lines(dt$speed, yscale_Wh_to_BatteryPct_PerUnitDistance*dt$WhPerUnitDistance_dir2, lwd=2, lty="dashed", col="gray")
    points(dt$speed, yscale_Wh_to_BatteryPct_PerUnitDistance*dt$WhPerUnitDistance_dir1, pch=20, col="gray")
    lines(dt$speed, yscale_Wh_to_BatteryPct_PerUnitDistance*dt$WhPerUnitDistance_dir1, lwd=2, lty="dotted", col="gray")
    
    coefs1 = apx$SpeedEnergyUsed$ord1
    intercept = yscale_Wh_to_BatteryPct_PerUnitDistance*coefs1$I
    slope = yscale_Wh_to_BatteryPct_PerUnitDistance*coefs1$M
    abline(a=intercept, b=slope, col=col_linear, lwd=lwd_linear, lty=lty_linear)
    
    coefs2 = apx$SpeedEnergyUsed$ord2
    x = seq(xlim[1], xlim[2]+10, length.out=50)
    y = yscale_Wh_to_BatteryPct_PerUnitDistance*(coefs2$C+(coefs2$A*x+coefs2$B)*x/coefs2$divisor)
...
plotR_DC_DEvsS = function(isPDF=FALSE)
    ...
    xlim = c(0, maxMeasuredSpeed_axis)
    xaxp = c(xlim, nTicksMeasuredSpeed_axis)
    ylim_left = c(0, maxRange_axis)
    yaxp_left = c(ylim_left, nTicksRange_axis)
    ylim_right_1 = c(0, maxDistancePerBattery1pct_axis)
    yaxp_right_1 = c(ylim_right_1, nTicksDistancePerBattery1pct_axis)
    ylim_right_2 = c(0, maxDistancePerKWh_axis)
    yaxp_right_2 = c(ylim_right_2, nTicksDistancePerKWh_axis)

    svpar = par(mai=par("mai")+c(0,0,0,ifelse(isPDF, 0.75, 1.25)), mgp=c(2.5, 1, 0))

    plot(NA, type='n', xlim=xlim, ylim=ylim_left, xaxp=xaxp, yaxp=yaxp_left, las=2, cex.main=ifelse(isPDF, 0.9, 0.8),
        main=tprintf("Est. Range plus Distance per 1% or KWh At Various Speeds, ~description@"),
        xlab=tprintf("Speed on Flat Road (~speed@)"),
        ylab=tprintf("Range of Full Battery (~dist_plural@)"))
    mtext(tprintf("(including linear approximation; assume battery degradation of ~batteryDegradation_pct@% ",
        "for capacity of ~batteryCapacity_KWh@ KWh, 100% state-of-charge is ~batteryCapacity_KWh@ KWh)"),
        side=3, line=ifelse(isPDF, 0.6, 0.5), cex=0.75)

    # Right-side y-axis 1.
    labels = pretty(ylim_right_1, yaxp_right_1[3])
    at = labels * yscale_DistancePerBattery1pct_to_Range
    axis(side=4, at=at, labels=labels, las=2, cex.axis=0.8)
    mtext(tprintf("~dist_plural@ per 1% battery"), side=4, line=1.75, cex=0.8)

    # Right-side y-axis 2.
    at = pretty(ylim_left, yaxp_left[3])
    axis(side=4, line=4, at=at, labels=FALSE, col.ticks="darkgray", tcl=1)
    labels = pretty(ylim_right_2, yaxp_right_2[3])
    at = labels * yscale_DistancePerKWh_to_Range
    axis(side=4, line=4, at=at, labels=labels, las=2, cex.axis=0.8)
    mtext(tprintf("~dist_plural@ per KWh"), side=4, line=5.75, cex=0.8)
    ...
    legend("bottomleft",
        c("Range and Distance/Energy vs Speed", paste0("Linear approx: ", coefs1$text)),
        col=c("black", col_linear), lwd=c(2, lwd_linear), lty=c("solid", lty_linear),
        pch=20, seg.len=4, cex=ifelse(isPDF, 1.2, 0.8))
    }
...
plotDvsSCE = function(isPDF=FALSE)
    ...
    mtxBatteryPct = matrix(NA, nrow=length(dt_batteryPlots$speed), ncol=length(dists),
        dimnames=list(as.character(dt_batteryPlots$speed), as.character(dists)))
    for (dist in dists)
        mtxBatteryPct[, as.character(dist)] = round(100*dist/dt_batteryPlots$range, 1)
    ...
    ylim_right = c(0, maxBatteryEnergy_right_axis)
    yaxp_right = c(ylim_right, nTicksBatteryEnergy_right_axis)
    svpar = par(mai=par("mai")+c(0,0,0,0.5), mgp=c(2.5, 1, 0))
    plot(NA, type='n', xlim=xlim, ylim=ylim_left, xaxp=xaxp, yaxp=yaxp_left, las=1, cex.main=ifelse(isPDF, 1, 1),
        main=tprintf("Battery Charge Used and Energy Used at Speeds/Distances, ~description@"),
        xlab=tprintf("Speed on Flat Road (~speed@)"),
        ylab="Battery Charge Used (%)")
    ...
    # Right-side y-axis.
    labels = pretty(ylim_right, yaxp_right[3])
    at = labels * yscaleBatteryEnergy_to_SOCpct
    axis(side=4, at=at, labels=labels, las=2)
    mtext(tprintf("Battery Energy Used (KWh)"), side=4, line=2.5)
    ...
    for (dist in colnames(mtxBatteryPct))
        {
        points(dt_batteryPlots$speed, mtxBatteryPct[,dist], pch=20)
        lines(dt_batteryPlots$speed, mtxBatteryPct[,dist], lwd=1)
        L = findSlope(dt_batteryPlots$speed, mtxBatteryPct[,dist])
        srt_deg = slopeToPlotAngle(L$slope)
        text(L$midX, L$midY, epaste(dist, tprintf("*' ~dist_plural@'")),
...
plotTvsSCE = function(isPDF=FALSE)
    ...
    mtxBatteryPct = matrix(NA, nrow=length(dt_batteryPlots$speed), ncol=length(elapsedHours),
        dimnames=list(as.character(dt_batteryPlots$speed), as.character(elapsedHours)))
    for (hours in elapsedHours)
        mtxBatteryPct[, as.character(hours)] = round(100*hours/dt_batteryPlots$hours, 1)
    mtxBatteryPct[mtxBatteryPct > 100] = NA
    ...
    svpar = par(mai=par("mai")+c(0,0,0,0.5), mgp=c(2.5, 1, 0))
    plot(NA, type='n', xlim=xlim, ylim=ylim_left, xaxp=xaxp, yaxp=yaxp_left, las=1, cex.main=ifelse(isPDF, 1, 1),
        main=tprintf("Battery Charge Used and Energy Used at Speeds/Times, ~description@"),
        xlab=tprintf("Speed on Flat Road (~speed@)"),
        ylab="Battery Charge Used (%)")
    mtext(tprintf("(assume battery degradation of ~batteryDegradation_pct@% for capacity of ~batteryCapacity_KWh@ KWh, ",
        "100% state-of-charge is ~batteryCapacity_KWh@ KWh)"),
    ...
    for (hours in colnames(mtxBatteryPct))
        {
        points(dt_batteryPlots$speed, mtxBatteryPct[,hours], pch=20)
        lines(dt_batteryPlots$speed, mtxBatteryPct[,hours], lwd=1)
        L = findSlope(dt_batteryPlots$speed, mtxBatteryPct[,hours], 0.8)
        srt_deg = slopeToPlotAngle(L$slope)
...
plotSvsDCE = function(isPDF=FALSE)
    ...
    yaxp_right = c(ylim_right, nTicksBatteryEnergy_right_axis)
    svpar = par(mai=par("mai")+c(0,0,0,0.5), mgp=c(2.5, 1, 0))
    plot(NA, type='n', xlim=xlim, ylim=ylim_left, xaxp=xaxp, yaxp=yaxp_left, las=2, cex.main=ifelse(isPDF, 1, 1),
        main=tprintf("Battery Charge Used and Energy Used at Distances/Speeds, ~description@"),
        xlab=tprintf("Distance Travelled (~dist_plural@)"),
        ylab="Battery Charge Used (%)")
    mtext(tprintf("(assume battery degradation of ~batteryDegradation_pct@% for capacity of ~batteryCapacity_KWh@ KWh, ",
        "100% state-of-charge is ~batteryCapacity_KWh@ KWh)"),
        side=3, line=ifelse(isPDF, 0.6, 0.5), cex=0.9)
...
plotSvsTCE = function(isPDF=FALSE)
    ...
    svpar = par(mai=par("mai")+c(0,0,0,0.5), mgp=c(2.5, 1, 0))
    plot(NA, type='n', xlim=xlim, ylim=ylim_left, xaxp=xaxp, yaxp=yaxp_left, las=2, cex.main=ifelse(isPDF, 1, 1),
        main=tprintf("Battery Charge Used and Energy Used at Times/Speeds, ~description@"),
...
plotDPMvsS = function(isPDF=FALSE)
    ...
    xlim = c(0, maxMeasuredSpeed_axis)
    xaxp = c(xlim, nTicksMeasuredSpeed_axis)
    ylim_left = c(0, maxDistPerUnitPrice_left_axis)
    yaxp_left = c(ylim_left, nTicksDistPerUnitPrice_left_axis)
    ...
    plot(NA, type='n', xlim=xlim, ylim=ylim_left, xaxp=xaxp, yaxp=yaxp_left, las=2, cex.main=ifelse(isPDF, 1.1, 1.0),
        main=tprintf("Distance per Unit Energy Cost, ~description@"),
        xlab=tprintf("Speed on Flat Road (~speed@)"),
        ylab=tprintf("Distance per Energy Cost (~distPerCost@)"))

    # Right-side y-axis.
    labels = pretty(ylim_right, yaxp_right[3])
    at = labels * yscaleDistancePer_FossilFuel_to_UnitPrice
    axis(side=4, at=at, labels=labels, las=2)
    ...
    lines(dt$speed, dt$electDistPerUnitPrice, col=col_elect, lty=lty_dist, lwd=lwd_elect)
    lines(dt$speed, dt$gasDistPerUnitPrice, col=col_fuel, lty=lty_dist, lwd=lwd_fuel)

    # Plotting distPerUnitFossilFuel means translating from the desired right-side y-axis to the left-side y-axis
    # used in plotting.
    distPerUnitFossilFuel = dt$distPerUnitFossilFuel * yscaleDistancePer_FossilFuel_to_UnitPrice
    ...
    legend("topright",
        legend=c(
            tprintf("Electricity distance per unit price (~distPerCost@)"),
            tprintf("Linear approximation: ", coefs$text),
            tprintf("Fuel consumption (~fuelEff@, right-side axis)"),
            tprintf("Fuel distance per unit price (~distPerCost@)")),
        col=c(col_elect, col_linear, col_dist_per_fuel, col_fuel),
        lwd=c(lwd_elect, lwd_linear, lwd_dist_per_fuel, lwd_fuel),
        lty=c(lty_dist, lty_linear, lty_dist_per_fuel, lty_dist),
        pch=c(20, NA, NA, NA),
        seg.len=4, cex=ifelse(isPDF, 0.55, 0.65))
...
plotMPDvsS = function(isPDF=FALSE)
    ...
    xlim = c(0, maxMeasuredSpeed_axis)
    xaxp = c(xlim, nTicksMeasuredSpeed_axis)
    ylim_left = c(0, maxCostPerUnitDist_left_axis)
    yaxp_left = c(ylim_left, nTicksCostPerUnitDist_left_axis)
    ylim_right = c(0, maxDistPerUnitFossilFuel_right_axis_2)
    yaxp_right = c(ylim_right, nTicksDistPerUnitFossilFuel_right_axis_2)
    
    # Make the main plot.
    svpar = par(mai=par("mai")+c(0,0.5,0.4,0.5), mgp=c(3, 1, 0))
    plot(NA, type='n', xlim=xlim, ylim=ylim_left, xaxp=xaxp, yaxp=yaxp_left, las=2, cex.main=ifelse(isPDF, 1.1, 1.0),
        main=tprintf("Energy Cost per Unit of Distance, ~description@"),
        xlab=tprintf("Speed on Flat Road (~speed@)"),
        ylab=tprintf("Energy Cost per Distance (~costPerDist@)"))

    # Right-side y-axis.
    labels = pretty(ylim_right, yaxp_right[3])
    at = labels * yscaleCostPerUnitDist_to_DistPerUnitFossilFuel
    axis(side=4, at=at, labels=labels, las=2)
    mtext(tprintf("Fuel Consumption (~fuelEff@)"), side=4, line=2.5)
    ...
    # Electricity and fuel DistPerUnitPrice at speeds.
    points(dt$speed, dt$electCostPerUnitDist, col=col_elect, pch=20)
    lines(dt$speed, dt$electCostPerUnitDist, col=col_elect, lty=lty_dist, lwd=lwd_elect)
    lines(dt$speed, dt$gasCostPerUnitDist, col=col_fuel, lty=lty_dist, lwd=lwd_fuel)

    # Plotting distPerUnitFossilFuel means translating from the desired right-side y-axis to the left-side y-axis
    # used in plotting.
    distPerUnitFossilFuel = dt$distPerUnitFossilFuel * yscaleCostPerUnitDist_to_DistPerUnitFossilFuel
    lines(dt$speed, distPerUnitFossilFuel, col=col_dist_per_fuel, lty=lty_dist_per_fuel, lwd=lwd_dist_per_fuel)
    ...
    legend("bottomleft",
        legend=c(
            tprintf("Fuel consumption (~fuelEff@, right-side axis)"),
            tprintf("Fuel cost per unit distance (~costPerDist@)"),
            tprintf("Electricity cost per unit distance (~costPerDist@)")),
        col=c(col_dist_per_fuel, col_fuel, col_elect),
        lwd=c(lwd_dist_per_fuel, lwd_fuel, lwd_elect),
        lty=c(lty_dist_per_fuel, lty_dist, lty_dist),
        pch=c(NA, NA, 20),
        seg.len=4, cex=ifelse(isPDF, 0.55, 0.65))
...
plotMvsS = function(isPDF=FALSE)
    ...
    ylim_right = c(0, maxDistPerUnitFossilFuel_right_axis_3)
    yaxp_right = c(ylim_right, nTicksDistPerUnitFossilFuel_right_axis_3)
    ...
    # Right-side y-axis.
    labels = pretty(ylim_right, yaxp_right[3])
    at = labels * yscaleDistancePer_FossilFuel_to_CostForDistHours
    axis(side=4, at=at, labels=labels, las=2)
    mtext(tprintf("Fuel Consumption (~fuelEff@)"), side=4, line=2.5)
    ...
    points(dt$speed, dt$electCostForFixedDistance, col=col_elect, pch=20)
    lines(dt$speed, dt$electCostForFixedDistance, col=col_elect, lty=lty_dist, lwd=lwd_elect)
    lines(dt$speed, dt$gasCostForFixedDistance, col=col_fuel, lty=lty_dist, lwd=lwd_fuel)
    ...
    # Plotting distPerUnitFossilFuel means translating from the desired right-side y-axis to the left-side y-axis
    # used in plotting.
    distPerUnitFossilFuel = round(dt$distPerUnitFossilFuel * yscaleDistancePer_FossilFuel_to_CostForDistHours, 1)
    lines(dt$speed, distPerUnitFossilFuel, col=col_dist_per_fuel, lty=lty_dist_per_fuel, lwd=lwd_dist_per_fuel)
    
    # Legend.
    legend("left",
        legend=c(
            tprintf("Fuel consumption (~fuelEff@, right)"),
            tprintf("Fuel cost per ", energyCostDist, " ~dist_plural@"),
            tprintf("Fuel cost per ", ifelse(energyCostHours == 1, "driving hour", paste0(energyCostHours, " driving hours"))),
            tprintf("Electricity cost per ", energyCostDist, " ~dist_plural@"),
            tprintf("Electricity cost per ", ifelse(energyCostHours == 1, "driving hour", paste0(energyCostHours, " driving hours")))),
        col=c(col_dist_per_fuel, col_fuel, col_fuel, col_elect, col_elect),
        lty=c(lty_dist_per_fuel, lty_dist, lty_time, lty_dist, lty_time),
        lwd=c(lwd_dist_per_fuel, lwd_fuel, lwd_fuel, lwd_elect, lwd_elect),
        pch=c(NA, NA, NA, 20, 20), seg.len=4, cex=0.65)
...
plotPvsS_AIR = function(isPDF=FALSE)
    ...
    mtext(tprintf("Assumptions:",
        ...
        side=3, line=0.75, cex=ifelse(isPDF, 0.75, 0.8))
    par(xaxp=xaxp)
    par(yaxp=yaxp)
    grid(col="darkgray", lwd=0.5, lty="solid")
...
plotCD_EDvsS_WIND = function(isPDF=FALSE)
    ...
    for (windSpeed in as.character(windSpeeds))
        ...
        mtx[windSpeed,] = yscale_Wh_to_BatteryPct_PerUnitDistance*1000*totalPowerKw/fineSpeeds
        }
    ...
    ylim_left = c(0, 1.3)
    yaxp_left = c(ylim_left, ylim_left[2]*10)
    ylim_right = c(0, ifelse(useMetricInPlots, 500, 800))
    yaxp_right = c(ylim_left, ylim_right[2]/100)
    svpar = par(mai=par("mai")+c(0,0.25,0.5,0.5), mgp=c(3, 1, 0))
    ...
    mtext(tprintf("Assumptions:",
        ...
        "\n",
        "  F: battery degradation ~batteryDegradation_pct@% for capacity of ~batteryCapacity_KWh@ KWh",
        "  G: 100% state-of-charge is ~batteryCapacity_KWh@ KWh"),
    ...
    for (windSpeed in windSpeeds)
        ...
        srt_deg = slopeToPlotAngle(L$slope)
        text(L$rightX*1.015, L$rightY, windSpeed, srt=srt_deg, adj=c(0.5, 0), cex=ifelse(isPDF, 0.5, 0.6))
...
plotR_DC_DEvsS_WIND = function(isPDF=FALSE)
    ...
    svpar = par(mai=par("mai")+c(0,0.15,0.5,1.25), mgp=c(3, 1, 0))

    plot(NA, type='n', xlim=xlim, ylim=ylim_left, xaxp=xaxp, yaxp=yaxp_left, las=2, cex.main=ifelse(isPDF, 0.9, 0.9),
        main=tprintf("Range plus Distance per 1% or KWh at Car and Wind Speeds, ~description@"),
        xlab=tprintf("Speed on Flat Road (~speed@)"),
        ylab=tprintf("Range of Full Battery (~dist_plural@)"))
    coefs = apx$SpeedPower$ord3
    mtext(tprintf("Assumptions:",
        ...
        "\n",
        "  F: 100% state-of-charge is ~batteryCapacity_KWh@ KWh",
        "  G: air density computed using density altitude"),
        side=3, line=0.4, cex=0.8)
    ...
    for (windSpeed in windSpeeds)
        ...
        text(L$midX, L$midY, S, srt=srt_deg, adj=c(0.5, -0.2), cex=ifelse(isPDF, 0.65, 0.8))
        }
    par(svpar)
    }
...
Plot charge power as a function of SOC (state-of-charge).

```{r fig.width=8.5, fig.height=5.5}
# CP = charge power, SOC = state-of-charge
plotCPvsSOC = function(isPDF=FALSE)
    {
    xlim = c(0, 100)
    xaxp = c(xlim, 20)
    ylim = c(0, maxChargePower_axis)
    yaxp = c(ylim, nTicksChargePower_axis)
    svpar = par(mai=par("mai")+c(0,0,0.25,0), mgp=c(3, 1, 0))
    plot(NA, type='n', xlim=xlim, ylim=ylim, xaxp=xaxp, yaxp=yaxp, las=2, cex.main=ifelse(isPDF, 1.1, 1.0),
        main=tprintf("Charge Power vs State-of-Charge at Maximum Charge Rate"),
        xlab=tprintf("State-of-Charge (%)"),
        ylab=tprintf("Charge Power (Kw)"))
    mtext(tprintf(
    ...
    par(yaxp=yaxp)
    grid(col="darkgray", lwd=0.5, lty="solid")
    lines(dfChargeCurve$SOCpct, dfChargeCurve$Power, lwd=2)
    par(svpar)
    }
...
Plot charge power as a function of charge time

```{r fig.width=8.5, fig.height=5.5}
# CP = charge power, CT = charge time
plotCPvsCT = function(isPDF=FALSE)
    {
    xlim = c(0, maxChargeMinutes_axis)
    xaxp = c(xlim, nTicksChargeMinutes_axis)
    ylim = c(0, maxChargePower_axis)
    yaxp = c(ylim, nTicksChargePower_axis)
    svpar = par(mai=par("mai")+c(0,0,0.25,0), mgp=c(3, 1, 0))
    plot(NA, type='n', xlim=xlim, ylim=ylim, xaxp=xaxp, yaxp=yaxp, las=2, cex.main=ifelse(isPDF, 1.1, 1.0),
        main=tprintf("Charge Power vs Charge Time at Maximum Charge Rate"),
    ...
    par(yaxp=yaxp)
    grid(col="darkgray", lwd=0.5, lty="solid")
    lines(dfChargeCurve$Minutes, dfChargeCurve$Power, lwd=2)
    par(svpar)
    }
...
Plot total battery energy (left y-axis) and state-of-charge (right y-axis) as a function of charge time

```{r fig.width=8.5, fig.height=5.5}
# E = energy, SOC = state-of-charge, CT = charge time
plotE_SOCvsCT = function(isPDF=FALSE)
    {
    xlim = c(0, maxChargeMinutes_axis)
    xaxp = c(xlim, nTicksChargeMinutes_axis)
    ylim_left = c(0, maxBatteryEnergy_right_axis)
    yaxp_left = c(ylim_left, nTicksBatteryEnergy_right_axis)
    ylim_right = c(0, maxSOCpct_axis)
    yaxp_right = c(ylim_right, nTicksSOCpct_axis)

    svpar = par(mai=par("mai")+c(0,0,0.25,0.5), mgp=c(2.5, 1, 0))
    plot(NA, type='n', xlim=xlim, ylim=ylim_left, xaxp=xaxp, yaxp=yaxp_left, las=2, cex.main=ifelse(isPDF, 1.1, 1.0),
        main=tprintf("Battery Energy and State-of-Charge vs Charge Time at Maximum Charge Rate"),
        xlab=tprintf("Charge Time (minutes)"),
        ylab=tprintf("Battery Energy (KWh)"))
    mtext(tprintf(
        "~chargeCurveDescription@",
        "\n",
        "~chargeCurveCitation@"),
    ...
    at = labels * yscaleBatteryEnergy_to_SOCpct
    axis(side=4, at=at, labels=labels, las=2)
    mtext(tprintf("State-of-Charge (%)"), side=4, line=2.5)

    par(xaxp=xaxp)
    par(yaxp=yaxp_left)
    grid(col="darkgray", lwd=0.5, lty="solid")

    # Plot a single curve which represents energy per time on the left axis and state-of-charge in percent on the right axis.
    lines(dfChargeCurve$Minutes, dfChargeCurve$TotalEnergy, lwd=2)
...
Do the exact inverse plot of the previous, using the RIGHT-SIDE y-axis for the x-axis.

```{r fig.width=8.5, fig.height=5.5}
# CT = charge time, SOC = state-of-charge
plotCTvsSOC = function(isPDF=FALSE)
    {
    xlim = c(0, 100)
    xaxp = c(xlim, 20)
    ylim = c(0, maxChargeMinutes_axis)
    yaxp = c(ylim, nTicksChargeMinutes_axis)
    svpar = par(mai=par("mai")+c(0,0,0.25,0), mgp=c(3, 1, 0))
    plot(NA, type='n', xlim=xlim, ylim=ylim, xaxp=xaxp, yaxp=yaxp, las=2, cex.main=ifelse(isPDF, 1.1, 1.0),
        main=tprintf("Charge Time vs. State-of-Charge at Maximum Charge Rate"),
        xlab=tprintf("State-of-Charge (%)"),
        ylab=tprintf("Charge Time (minutes)"))
    mtext(tprintf(
        "~chargeCurveDescription@",
        "\n",
        "~chargeCurveCitation@"),
    ...
    par(yaxp=yaxp)
    grid(col="darkgray", lwd=0.5, lty="solid")
    lines(dfChargeCurve$SOCpct, dfChargeCurve$Minutes, lwd=2)
    par(svpar)
    }
